  ...
  const profileRows = [
    {
      icon: nameIcon,
      label: '이름',
      value: profile?.name ?? '김**',
    },
    {
      icon: idIcon,
      label: 'ID',
      value: profile?.id ?? 'Parents1',
    },
    {
      icon: passwordIcon,
      label: '비밀번호',
    ...
    <main className="relative mx-auto flex min-h-screen w-full max-w-[402px] flex-col bg-[#EDEAF5] pb-[67px]">
      ...
      {profile?.photo ? (
        <img
          src={profile.photo}
          alt="선택한 프로필"
          className="mx-auto mt-[39px] size-[96px] rounded-full object-cover"
        />
      ) : (
        <img
          src={profileIcon}
          alt="프로필"
          className="mx-auto mt-[39px] size-[96px]"
        />
      )}

      {/* Profile information + logout */}
      <section
        className="mx-auto mt-[62px] w-[362px]"
        aria-label="프로필 정보"
      >
        {/* Profile information */}
        <div className="mx-auto w-[282px] space-y-[15px]">
          {profileRows.map((row) => (
            <div
              key={row.label}
              className="flex h-[39px] items-center gap-5"
            >
              <img
                src={row.icon}
                alt=""
              ...
              <span className="whitespace-nowrap text-[16px] font-normal text-[#1C1B1F]">
                {row.label} | {row.value}
              </span>
            </div>
          ))}
        ...
        <button
          ...
          className="mt-[53px] flex h-[80px] w-full items-center gap-2 rounded-[20px] bg-[#FBF5FF] px-[15px] text-[16px] font-medium tracking-[-0.8px] text-[#1C1B1F]"
        >
          <img
            src={logoutIcon}
            alt=""
      ...
      <button
        type="button"
        onClick={onEdit}
        className="mx-auto mt-auto h-[50px] w-[341px] rounded-[10px] bg-[#31302E] text-[16px] font-semibold text-white"
      >
        수정
      </button>
