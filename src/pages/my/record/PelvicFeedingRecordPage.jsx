...
const ChoiceButton = ({
  ...
  <button
    type="button"
    onClick={onClick}
    className={`flex h-[67px] items-center justify-center rounded-[10px] px-2 text-[clamp(14px,4.98vw,20px)] font-medium tracking-[-0.02em] ${
      isSelected
...
const SeveritySlider = ({ label, value, onChange }) => (
  <section>
    <div className="flex items-center justify-between">
      <h2 className="text-[20px] font-medium leading-[30px] tracking-[-0.4px]">
        {label}
      </h2>

      <span className="text-[20px] font-medium leading-[30px] tracking-[-0.4px]">
        {value}/5
      </span>
    </div>
    ...
    <div className="relative mt-[21px] h-4 w-[262px] max-w-full">
      ...
      {/* 현재 값까지 보라색 */}
      <div
        className="absolute left-0 top-1/2 h-[7px] -translate-y-1/2 rounded-full bg-primary"
        style={{
          width: `${(value / 5) * 100}%`,
        }}
      />
...
const PelvicFeedingRecordPage = ({ onBack, onNext }) => {
  /*
   * 처음 기록하는 경우:
   * 선택형 버튼은 아무것도 선택하지 않음.
   */
  const [pelvicSymptom, setPelvicSymptom] = useState(null);
  ...
  const [milkPain, setMilkPain] = useState(0);
      ...
      <div className="px-[clamp(24px,9vw,36px)] pt-[10px]">
        ...
        <section>
          ...
          <div className="-mx-[10px] mt-[15px] grid grid-cols-4 gap-[6px]">
            {pelvicSymptoms.map((symptom) => (
              <ChoiceButton
                key={symptom}
                isSelected={pelvicSymptom === symptom}
                onClick={() => setPelvicSymptom(symptom)}
                className={`rounded-[15px] ${
                  symptom === '복부 갈라짐'
                    ? 'whitespace-nowrap px-0 text-[6px]'
                    : symptom === '복부 처짐'
                      ? 'whitespace-nowrap px-0 text-[10px]'
                      : 'text-[clamp(12px,3.98vw,16px)]'
                }`}
              >
                {symptom}
              </ChoiceButton>
          ...
          <div className="mt-[29px] grid grid-cols-3 gap-[10px]">
            {feedingMethods.map((method) => (
              <ChoiceButton
                ...
                onClick={() => setFeedingMethod(method)}
              >
                {method}
              </ChoiceButton>
            ))}
        ...
        <div className="mt-[133px] flex min-h-20 items-center gap-4 rounded-[20px] bg-primary-background px-[31px] py-3">
          ...
          <p className="text-[12px] font-medium leading-4 tracking-[-0.48px] text-primary">
            입력을 완료하였거나 건너뛰고 싶을 경우,
            <br />
            다음 버튼을 눌러 페이지를 이동해주세요.
          </p>
        </div>
        ...
        <div className="-mx-[13px] mt-4 flex gap-[19px]">
          <button
            type="button"
            onClick={onBack}
            className="h-[51px] flex-1 rounded-[10px] bg-gray-50 text-[16px] text-[#31302e]"
          >
            이전
          </button>
          ...
          <button
            ...
            className="h-[51px] flex-1 rounded-[10px] bg-[#31302e] text-[16px] text-white"
          >
            다음
          </button>
        </div>
