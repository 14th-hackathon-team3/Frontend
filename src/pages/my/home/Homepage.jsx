...
import progressCurve from '../../../assets/Vector.svg';
import progressGuide from '../../../assets/line.svg';
  ...
  return (
    <main className="relative mx-auto min-h-screen w-full max-w-[402px] overflow-hidden bg-primary-light pb-[118px] pt-[55px]">
      ...
      <section className="relative mt-0 h-[286px] overflow-hidden" aria-label="회복 여정 Day 36">
        <img src={heroBackground} alt="" className="absolute -left-[21px] top-[32px] h-[310px] w-[402px]" />
        <img src={dayArc} alt="" className="absolute -left-[16px] top-[50px] h-[178px] w-[402px]" />
      ...
      <button type="button" onClick={() => setIsRecoveryInfoOpen(true)} className="mx-auto -mt-[22px] block h-[120px] w-[358px] rounded-[24px] bg-gray-50 px-5 text-left shadow-sm" aria-label="산후 회복 진행 정보 보기">
        <div className="relative h-full"><img src={progressGuide} alt="" className="absolute left-[25px] top-[28px] h-[32px] w-[267px]" /><img src={progressCurve} alt="" className="absolute left-[25px] top-[27px] h-[34px] w-[162px]" />{weekSteps.map((step) => <span key={step.label} className={`absolute top-[48px] ${step.position} flex flex-col items-center text-[9px] ${step.current ? 'font-bold text-[#7b4f9f]' : 'text-[#9ba89a]'}`}><span className={`mb-1 flex size-[13px] items-center justify-center rounded-full ${step.color} ${step.current ? 'ring-4 ring-[#ead4ff]' : ''}`}>{step.completed && <span className="text-[9px] leading-[13px] text-white">✓</span>}</span>{step.label}</span>)}</div>
      </button>
