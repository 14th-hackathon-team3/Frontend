...
const isMissingPrivateField = (log, field) =>
  Boolean(log) && !hasOwnField(log, field);
...
const TRACKING_VISIBILITY_STORAGE_KEY =
  'journey.hiddenTrackingCategories';
...
const toChartPoints = (
  ...
  return values.map((value) => {
    ...
    const normalized =
      (Number(value) - resolvedMin) / range;
...
const createRecords = (log, dateKey) => {
  ...
  const emotionPrivate =
    isMissingPrivateField(log, 'emotion');

  const sleepPrivate =
    isMissingPrivateField(log, 'sleep_hours');

  const painScorePrivate =
    isMissingPrivateField(log, 'pain_score');

  const painAreaPrivate =
    isMissingPrivateField(
      log,
      'pelvic_floor_symptoms',
    ) &&
    isMissingPrivateField(log, 'pain_area');

  const breastfeedingPrivate =
    isMissingPrivateField(
      log,
      'breastfeeding',
    );

  const skinScorePrivate =
    isMissingPrivateField(
      log,
      'skin_self_score',
    );

  const skinTagsPrivate =
    isMissingPrivateField(
      log,
      'skin_symptom_tags',
    );
  ...
  const exercisePrivate =
    isMissingPrivateField(log, 'exercise');

  const memoPrivate =
    isMissingPrivateField(log, 'memo');
      ...
      {record.sections.map(
        (section, index) => (
          <div key={section.label}>
            ...
            {section.isPrivate ? (
              <p className="mt-4 text-[18px] font-medium leading-7 text-gray-400">
                비공개 처리됨
              </p>
            ) : section.values ? (
...
const WeeklyJourneyPage = ({
  ...
  const latestSleep =
    getLatestNumber(sleep);

  const latestPain =
    getLatestNumber(pain);
    ...
    return (
      <WeeklyJourneyPage
        ...
        onConfirmPrivate={async (
          ...
          const currentlyPrivate =
            privateCards.includes(
              card,
            );

          const nextPrivateCards =
            currentlyPrivate
              ? privateCards.filter(
                  (item) =>
                    item !==
